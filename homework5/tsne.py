...
def pca(X = np.array([]), initial_dims = 50):
    ...
    Returns:
    - Y: An array with shape N x no_dims where N is the number of examples and initial_dims is the
...
def compute_gradient(P, Q, Y_sim, Y, dY, no_dims):
    ...
    - Y: An array with shape N x no_dims, where N is the number of examples and no_dims
         is the dimensionality of lower dimensional data Y.
    - dY: An array with same shape of Y, where dY[i][j] is the same shape of 
    ...
    n = Y.shape[0]
    for i in range(n):
        dY[i,:] = 4 * np.sum(np.tile(PQ[:,i] * Y_sim[:,i], (no_dims, 1)).T * (Y[i,:] - Y), 0)
    return dY
...
if __name__ == "__main__":
    ...
    labels = np.loadtxt("mnist2500_labels_int.txt")
    Y = tsne(X, 2, 50, 20.0)
