...
def objective_function(X, y, w, lamb):
    ...
    Returns:
    - objective function in SVM primal formulation
    """
    # you need to fill in your solution here

    l_hat = 1 - y.reshape([-1, 1]) * np.dot(X, w)
    l = (l_hat > 0) * l_hat
    obj_value =  lamb / 2. * np.linalg.norm(w) ** 2 + np.mean(l)
    return obj_value
...
def pegasos_mnist():
    ...
    for lamb in (0.01, 0.1, 1):
        ...
        # plt.plot(train_obj['k=' + str(k) + '_lambda=' + str(lamb)])
        # plt.show()
